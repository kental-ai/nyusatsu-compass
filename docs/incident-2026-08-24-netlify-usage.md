...
## 続報: ビルド時間上限の超過（2026-08-29）

8/29、アーカイブ実装・品質パスを含むビルドがNetlifyで完了しなくなった（本番は前日分のまま）。
ローカルのクリーン環境では全工程成功（EXIT=0）のため環境固有と判断。
主因は**ビルド所要時間**: GEPS全年度zip14本を毎ビルドでダウンロードしており、データ増で上限を超えた。

対策（実施済み・効果はクリーン環境実測で13分→5分8秒）:
- GEPS過去年度（〜2024・265,118行）を凍結スナップショット `data/snapshots/geps_history.csv.gz`（13MB）に
- ビルドは `snapshot_geps.mjs import` + `fetch_geps.mjs recent`（直近2年のみ取得）に変更
- あわせて `NODE_OPTIONS=--max-old-space-size=6144` を明示（データ増へのOOMヘッジ）
- 年度が進んだら snapshot_geps export をやり直す（年1回）

学び: ビルドは「時間・メモリ・デプロイ回数」の3つの枠を持つ。データが毎日太る設計なので、
どの枠も「今ギリギリか」を週次監査で見える化する（audit にビルド時間の記録を検討）。
